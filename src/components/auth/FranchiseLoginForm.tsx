...
const FranchiseLoginForm: React.FC = () => {
  ...
  const validateField = (name: string, value: string) => {
    ...
    if (name === "password") {
      if (value.length > 0 && (value.length < 6 || value.length > 10)) {
        error = "Password must be 6–10 characters";
      }
    }
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "phoneNumber" && value && !/^\d*$/.test(value)) {
      return; // silently ignore non-digits
    }
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.phoneNumber.trim() !== formData.phoneNumber) {
      newErrors.phoneNumber = "Remove spaces from phone number";
    }
    if (formData.password.trim() !== formData.password) {
      newErrors.password = "Remove spaces from password";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.result?.token) {
        ...
        throw new Error(response.error?.[0] || response.message || "Invalid credentials");
      }
    } catch (err: any) {
  ...
  return (
    <main className="mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className={`mt-1 w-full border rounded-md p-2.5 focus:ring-blue-500 focus:border-blue-500 ${
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full border rounded-md p-2.5 pr-10 focus:ring-blue-500 focus:border-blue-500 ${
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 text-white font-semibold py-2.5 rounded-md hover:bg-blue-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
          >
      ...
      <p className="mt-6 text-sm text-center text-gray-600 flex justify-center items-center gap-1">
        Don't have an account?{" "}
        <Phone className="text-green-500" size={16} />
        <a
          href="tel:+919603558369"
