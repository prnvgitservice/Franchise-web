...
const FranchiseLoginForm: React.FC = () => {
  ...
  const validateInputs = (data: LoginData): string | null => {
    ...
    if (data.phoneNumber.trim() !== data.phoneNumber || data.password.trim() !== data.password) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.response?.status === 401) {
        errorMessage = response.error?.[0] || 'Invalid phone number or password';
      } else if (err.response?.status === 400) {
        errorMessage = response.error?.[0] || 'Bad request. Please check your inputs.';
